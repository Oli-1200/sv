...
def compare_images(aligned, ref_img, config, debug_path=None, is_snl=False):
    ...
    logger.debug(f"soft={count_soft}px (>{max_soft}?) | hard_center={hard_pixels}px (>{max_hard}?) | is_snl={is_snl}")
...
def process_one(image_path, export_root):
    ...
    logger.debug(f"{filename} essai1(no_snl=True) → num={num1}, ref={ref1}")
    ...
    if num1 is not None and ref1 is not None:
        ...
        logger.debug(f"{filename} essai2(no_snl=False) → num={num2}, ref={ref2}")
        ...
        if num2 is not None and ref2 is not None:
            ...
            num_liste = "SNL"
            logger.warning(f"{filename} : aucun numero lisible → fallback SNL")

    if ref_name is None:
        logger.error(f"{filename} : pas de bulletin de référence disponible (même pas SNL), on saute.")
        return
    ...
    if ref_img is None:
        logger.error(f"Impossible de lire le bulletin de reference {ref_path}")
        return
    ...
    logger.info(f"{filename} → {status} (soft={counts['soft']}, hard={counts['hard']}) ref={ref_name}")
