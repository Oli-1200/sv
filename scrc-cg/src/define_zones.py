...
def save_config(cfg):
    ...
    logger.info(f"Config sauvegardée dans {CONFIG_PATH}")
...
def main(image_path):
    ...
    while True:
        ...
        if k in [ord('q'), 27]:
            ...
            logger.info(f"Zone d'exclusion ajoutée: x={x}, y={y}, w={w}, h={h}")
        elif k == ord('i') and current_rect:
            x, y, w, h = rect_to_xywh(current_rect)
            identification = {"x": x, "y": y, "w": w, "h": h}
            cfg["zones"]["identification"] = identification
            logger.info(f"Zone d'identification définie: x={x}, y={y}, w={w}, h={h}")
        elif k == ord('s'):
            save_config(cfg)
        elif k == ord('+'):
