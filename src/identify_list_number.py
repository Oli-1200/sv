...
def identify_list_number(img_bgr, config, debug=False, no_snl=False):
    ...
    if debug:
        logger.debug(f"{len(digit_imgs)} chiffre(s) trouvé(s)")
    ...
    for i, dimg in enumerate(digit_imgs):
        ...
        if debug:
            logger.debug(f"digit {i+1} → {digit} (score={score:.2f})")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(img_path):
        logger.error(f"Image introuvable : {img_path}")
        exit(1)

    config = load_config()
    img = cv2.imread(img_path)
    if img is None:
        logger.error(f"Impossible de lire {img_path}")
        exit(1)

    code, ref = identify_list_number(img, config, debug=args.debug, no_snl=args.no_snl)
    logger.info(f"Liste détectée : {code}")
    logger.info(f"Fichier de référence : {ref}")