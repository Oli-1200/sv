...
def detect_corners(image_bgr, config, debug_name=None):
    ...
    if debug_name:
        print(f"[DEBUG] detect_corners sur {debug_name} ({w_img}x{h_img})")
    ...
    for name, box in boxes.items():
        ...
        if roi.size == 0:
            print(f"[WARN] zone vide pour {name} dans {debug_name or ''} : x={x}, y={y}, w={w}, h={h}")
        ...
        if max_val < threshold:
            print(f"[WARN] corrélation faible pour {name} ({debug_name or ''}): {max_val:.2f}")
